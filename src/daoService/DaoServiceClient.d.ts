declare namespace ds {
  ...
  enum mode {
    WHERE_FILTER_EMPTY = 'basic-actions.read.where-filter-empty',
    WHERE_FILTER_NULL = 'basic-actions.read.where-filter-null',
  }
  ...
  export interface FullyPersistSupport<M, PK> {
    /**
     ...
     * 禁用通用字段覆盖值
     * @example
     *  // 保存时自行赋值部门对象的创建人和创建时间, 持久化时不会被覆盖
     *  employee.offCoverCommonFields(employee.dept, dept.creator, dept.createDatetime)
     *    .deep()
     *    .save(record);
     *
     * @param model 持久化对象树中的某个模型
     * @param fields 该模型被禁用的通用字段
     */
    offCoverCommonFields(
        model: ModelExpression<any, any>,
        ...fields: Array<FieldExpression<any>>
    ): FullyPersistSupport<M, PK>;
  ...
  export interface DeleteSupport<M, PK> {
    /**
     * 删除数据
     *
     * @param records 数据集
     ...
     * @example
     *  // 删除用户时, 级联删除用户的地址数据
     *  QUser.user.cascaded(QUser.addressInfo).delete(user);
     *
     * @param models 关联模型表达式
     */
    cascaded(...models: Array<ModelExpression<any, any>>): DeleteSupport<M, PK>;
  }

  export abstract class ModelExpression<M, PK>
      implements FullyPersistSupport<M, PK>, DeleteSupport<M, PK> {
    /**
     * @private
     */
     ...
     *
     * employee.employeeId.hqlFragment(); // => employeeId
     * employee.dept.c(dept.deptName); // => dept.deptName
     *
     * @param field 联结字段
     */
    c(field: FieldExpression<any>): FieldExpression<any>;
     ...
     * employee.employeeId.hqlFragment(); // => employeeId
     * employee.dept.c(dept.deptName); // => dept.deptName
     * employee.c(employee.roles).c(role.resources).resourceUrl; // => roles.resources.resourceUrl
     * @param field 关联模型
     */

    c<O, PK>(field: ModelExpression<O, PK>): ModelExpression<O, PK>;
    ...
    tag(tag: string): this;
      ...
      /**
       * 用作校验的 validationGroupName.
       * @example
       * const employee = ds.QEmployee;
       * // 若定义了 validationGroupName 则优先通过 validationGroupName 查找校验分组.
       * employee
       *    .validationGroupName('updateEmployeeEmail')
       *    .update(employee.email)
       *    .execute({
       *        email: 'user1@gmail.com',
       *        confirmEmail: 'otherUser@gmail.com'
       ...
       */
      validationGroupName(validationGroupName: string): this;
       ...
       * const employee = ds.QEmployee
       *
       * employee.mode(ds.mode.WHERE_FILTER_EMPTY)
       *  .select()
       *  .where(employee.username.eq$(param.username).and(employee.email.eq$(param.email)))
       *  .execute();
     *
     * @param mode 执行模式配置项
     */
    mode(...mode: Array<ds.mode>): this;
  ...
  abstract class AbstractSelectCommand<M,
      PK,
   ...
   */
  class SelectCollectCommand<M, PK> extends AbstractSelectCommand<M,
      PK,
  ...
  class SelectPageCommand<M, PK> extends AbstractSelectCommand<M,
      PK,
  ...
  class SelectNotCountPageCommand<M, PK> extends AbstractSelectCommand<M,
      PK,
  ...
  class SelectOneCommand<M, PK> extends AbstractSelectCommand<M,
      PK,
  ...
  class AddBatchCommand<M, PK> extends BaseCommand<M,
      PK,
   ...
   * @deprecated
   */
  class CustomCommand<M, PK> extends BaseCommand<M, PK, CustomCommand<M, PK>> {
    execute(): Promise<{ success: boolean; data: NotCountPage<M> }>;
  ...
  class BaseCustomizationCommand<C extends BaseCustomizationCommand<C, R>, R> {
      validationGroupName(validationGroupName: string): this;

      execute(arguments: object): Promise<{ success: boolean; data: R }>;
    execute(): Promise<{ success: boolean; data: R }>;
  }

  class CustomizationSelectOneCommand<T> extends BaseCustomizationCommand<CustomizationSelectOneCommand<T>,
      T> {}

  class CustomizationSelectCommand<T> extends BaseCustomizationCommand<CustomizationSelectCommand<T>,
      Array<T>> {
    /**
     * 进行分页查询
     *
     ...
     * @param pageSize 每页大小
     */
    paging(currentPage: number, pageSize: number): CustomizationPageCommand<T>;

    /**
     * 声明该自定义查询的排序信息
     *
     * @example
     * .sorting('username') //根据用户名正序排序
     * @example
     * .sorting('username:DESC') //根据用户名排序倒序排序
     * @example
     * .sorting('username:DESC NULL FIRST') //根据用户名排序倒序排序, 且空值排在前面
     * @example
     * .sorting('username NULL LAST') //根据用户名排序正序排序, 且空值排在后面
     *
     * @param orderExpressions 排序表达式
     */
    sorting(...orderExpressions: Array<string>): CustomizationSelectCommand<T>;
  }

    class CustomizationSelectCountCommand<T> extends BaseCustomizationCommand<CustomizationSelectCountCommand<T>,
        Array<T>> {
    }

  class CustomizationPageCommand<T> extends BaseCustomizationCommand<CustomizationPageCommand<T>,
      Page<T>> {
    /**
     * 将分页查询转换为不统计总数的分页查询
     */
    doNotNeedCount(): CustomizationNotCountPageCommand<T>;
  }

  class CustomizationNotCountPageCommand<T> extends BaseCustomizationCommand<CustomizationNotCountPageCommand<T>,
      NotCountPage<T>> {}

  class CustomizationPersistCommand extends BaseCustomizationCommand<CustomizationPersistCommand,
      number> {}

  class CustomizationCommand {
    /**
     * 查询列表操作
     * @example
     * dsc.customization('selectEmployeePage')
     *  .paging(param.current, param.pageSize)
     *  .sorting('username NULL LAST')
     *  .execute({username: param.searchKey});
     */
    select(): CustomizationSelectCommand<object>;

      /**
       * 查询列表操作(过滤输出字段)
       * @example
       * dsc.customization('selectEmployeePage')
       *  .select(QEmployee.username,QEmployee.age)
       *  .execute();
       */
      select(...fields: Array<FieldExpression<any>>): CustomizationSelectCommand<object>;

    /**
     * 查询单条数据操作
     * @example
     * dsc.customization('selectPayedFirstMaleCustomer')
     *  .selectOne()
     *  .execute()
     */
    selectOne(): CustomizationSelectCommand<object>;


      /**
       * 查询单条数据操作(过滤输出字段)
       * @example
       * dsc.customization('selectEmployee')
       *  .selectOne(QEmployee.username,QEmployee.age)
       *  .execute();
       */
      selectOne(...fields: Array<FieldExpression<any>>): CustomizationSelectCommand<object>;

    /**
     * 自定义SQL持久化操作, 返回影响行数
     */
    persist(): CustomizationPersistCommand;

      /**
       * 自定义SQL查询count计数, 返回count数
       */
      selectCount(): CustomizationSelectCountCommand<object>;
  }

  class UpdateBatchCommand<M, PK> extends BaseCommand<M,
      PK,
  ...
  class DaoServiceClient {
    ...
    /**
     * @deprecated 请使用 client.customization() 方式
     */
    customCommand<M, K>(customApiKey: string): CustomCommand<M, K>;

    /**
     * @deprecated 请使用 client.customization().select() 方式
     */
    customSelect(
        customSelectApiKey: string,
    ): CustomizationSelectCommand<object>;

    /**
     * @deprecated 请使用 client.customization().persist() 方式
     * 自定义sql增删改
     * @param customPersistApiKey 自定义sql的tag
     */
    customPersist(customPersistApiKey: string): CustomizationPersistCommand;

    /**
     * 开始自定义指令定义
     *
     * @param sqlKey 自定义SQL 唯一识别键
     */
    customization(sqlKey: string): CustomizationCommand;
  }
}
