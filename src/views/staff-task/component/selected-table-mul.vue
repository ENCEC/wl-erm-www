    ...
    <el-form ref="tableFormRef" :model="tableForm" class="tableform-wrap" size="mini" label-width="auto" :rules="tableFormRules">
      ...
      >
        ...
        <template v-if="type==='detail'">
          <el-table-column prop="endDate" label="实际完成日期" width="100" />
          <el-table-column prop="progress" label="完成进度(%)" min-width="100" />
          <el-table-column prop="resultAccess" label="完成结果" />
        ...
        >
          <template slot-scope="scope">
            <el-button v-if="!scope.row.isNeed" type="text" size="mini" :disabled="scope.row.isNeed" @click="handleDelete(scope.row.standardDetailId)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 表格 End -->
  ...
  methods: {
    ...
    handleDelete(id) {
      this.tableForm.tableData.forEach((item, index) => {
        if (item.standardDetailId === id) {
          this.tableForm.tableData.splice(index, 1)
          // 因为this.tableForm.tableData 和records 是浅拷贝
          // 所以操作this.tableForm.tableData相当于操作records
          // 所以不需要再发送方法操作 更新删除后的数据
